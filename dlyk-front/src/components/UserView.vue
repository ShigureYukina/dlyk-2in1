<template>
  ...
  <el-dialog v-model="UserdialogVisible" :title="userQuery.id>0 ?'编辑用户' : ' 新增用户'" center width="55%" draggable>
...
export default defineComponent({
  ...
  methods: {
    ...
    del(id) {
      messageConfirm("你确定要删除该用户吗？").then(() => {
        return doDelete("/api/user/" + id, {}).then(response => {
          if (response.data.code === 200) {
            messageTip("删除用户成功", "success");
            this.reload(); // 刷新用户列表
          } else {
            messageTip("删除用户失败", "error"); // 使用 messageTip 保持一致性
          }
        }).catch(error => {
    ...
    userSubmit() {
      ...
      this.$refs.loginRefForm.validate((valid) => {
        if (valid) {
          ...
          // 根据 userQuery.id 判断是编辑还是新增
          const apiUrl = this.userQuery.id > 0 ? "/api/user" : "/api/user"; // 确保 URL 正确

          const request = this.userQuery.id > 0 ? doPut(apiUrl, formData) : doPost(apiUrl, formData);

          request.then(response => {
            if (response.data.code === 200) {
              const actionMessage = this.userQuery.id > 0 ? "编辑用户成功" : "新增用户成功";
              ...
              const errorMessage = this.userQuery.id > 0 ? "编辑用户失败" : "新增用户失败";
              this.$message.error(errorMessage);
            }
          }).catch(error => {
            console.error(this.userQuery.id > 0 ? "编辑用户失败:" : "新增用户失败:", error);
            this.$message.error(this.userQuery.id > 0 ? "编辑用户失败" : "新增用户失败");
          });
        } else {
          // 验证不通过，提示用户
